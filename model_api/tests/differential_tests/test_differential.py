...
def test_model_prediction_differential(*, save_file: str = 'test_data_predictions.csv'):
    ...
    # Given : Load the saved previous model predictions
    ...
    # Then : diff the current model vs. the old model
    ...
    for previous_value, current_value in zip(previous_model_predictions, current_model_predictions):
        previous_value = previous_value.item()  # convert numpy float64 to Python float.
