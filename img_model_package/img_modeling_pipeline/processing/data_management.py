...
def load_single_image(data_folder: str, filename: str) -> pd.DataFrame:
    ...
    image_path = os.path.join(data_folder, f'{filename}')
    images_df = pd.DataFrame([image_path, 'unknown'], index=['image', 'target']).T
    # image_df = []
    #
...
def save_pipeline_keras(model) -> None:
    ...
    joblib.dump(model.named_steps['datasets'], config.PIPELINE_PATH)
    joblib.dump(model.named_steps['model'].classes_, config.CLASSES_PATH)
    model.named_steps['model'].model.save(str(config.MODEL_PATH))
